...
public class UIManager {
    ...
    TextField[] xyInput = new TextField[2];
    List taskbar;
    List viewControls;
    public Button playBackButton;
    public Button pauseButton;
    public Button stepButton;
    List playBackControls;
    List shapeProperties;            // density, friction, restitution, isSensor, edit
    List bodyProperties;            // name, userdata, type, isBullet, edit, tex_file, tex_width, tex_height
    List jointProperties;            // name, userdata, type, collideConnected, joint_specific_parameters
    List jointPropertyRows;

    public UIManager(SceneManager manager,Button playButton,Button pauseButton,Button stepButton) {
        this.playBackButton = playButton;
        this.pauseButton = pauseButton;
        this.stepButton = stepButton;

        this.sceneManager = manager;
        this.taskbar = new ArrayList();
        this.viewControls = new ArrayList();
        this.playBackControls = new ArrayList();
        this.shapeProperties = new ArrayList();            // density, friction, restitution, isSensor, edit
        this.bodyProperties = new ArrayList();            // name, userdata, type, isBullet, edit, tex_file, tex_width, tex_height
        this.jointProperties = new ArrayList();            // name, userdata, type, collideConnected, joint_specific_parameters
        this.jointPropertyRows = new ArrayList();
    }
...
//            alertDialog.hide();
//            var data = new Blob([JSON.stringify(sceneManager.saveScene(), null, 4)], {
//                type:
//                'text/plain'
//            });
...
//                    var data = new Blob([JSON.stringify(sceneManager.exportWorld(), null, 4)], {
//                        type:
//                        'text/plain'
//                    });
//                    var textFile = window.URL.createObjectURL(data);
...
//                    var data = new Blob([JSON.stringify(sceneManager.exportSelection(), null, 4)], {
//                        type:
//                        'text/plain'
//                    });
//                    var textFile = window.URL.createObjectURL(data);
...
//        this.updateJointPropertyView();
    ...
    public void onMouseDown (InputHandler inputHandler) {
        this.updateShapePropertyView();
        this.updateBodyPropertyView();
        this.updateJointPropertyView();

        // update input-xy
        if (inputHandler.selection.size() != 1) {
            this.xyInput[0].setText("");
            this.xyInput[1].setText("");
            return;
        }
    ...
    public void onMouseMove (InputHandler inputHandler) {
        this.updateShapePropertyView();
        this.updateBodyPropertyView();
        this.updateJointPropertyView();
    ...
    public void onMouseUp (InputHandler inputHandler) {
        this.updateShapePropertyView();
        this.updateBodyPropertyView();
        this.updateJointPropertyView();
    ...
    public void updateShapePropertyView () {
//        SceneManager sceneManager = this.sceneManager;
//        if ((sceneManager.state == SceneManager.STATE_BODY_EDIT_MODE ||
//                sceneManager.state == SceneManager.STATE_SHAPE_EDIT_MODE) &&
//                sceneManager.selectedShapes.size() > 0) {
//            $("#shape_properties").show();
//
//            if (sceneManager.selectedShapes.size() == 1) {
//                this.shapeProperties[0].disabled = false;
//                this.shapeProperties[0].value = sceneManager.selectedShapes.get(0).density;
//                this.shapeProperties[1].value = sceneManager.selectedShapes.get(0).friction;
//                this.shapeProperties[2].value = sceneManager.selectedShapes.get(0).restitution;
//                this.shapeProperties[3].value = sceneManager.selectedShapes.get(0).maskBits;
//                this.shapeProperties[4].value = sceneManager.selectedShapes.get(0).categoryBits;
//                this.shapeProperties[5].value = sceneManager.selectedShapes.get(0).groupIndex;
//                this.shapeProperties[6].checked = sceneManager.selectedShapes.get(0).isSensor;
...
//
//                boolean allAreSensor = false;
//                for (int i = 0; i < sceneManager.selectedShapes.size(); i++) {
//                    if (allAreSensor != sceneManager.selectedShapes.get(i).isSensor && i != 0) {
//                        allAreSensor = false;
//                        break;
//                    } else {
//                        allAreSensor = sceneManager.selectedShapes.get(i).isSensor;
//                    }
//                }
...
//        }
    ...
    public void updateBodyPropertyView () {

//        if ((sceneManager.state == SceneManager.STATE_DEFAULT_MODE ||
//                sceneManager.state == SceneManager.STATE_BODY_EDIT_MODE) &&
//                sceneManager.selectedBodies.size() > 0) {
//            $("#body_properties").show();
//
//            if (sceneManager.selectedBodies.size() == 1) {
//                Body cachedBody = sceneManager.selectedBodies.get(0);
...
//                    this.bodyProperties[10].value = cachedBody.getSpriteSourceWidth() != 0 ? cachedBody.getSpriteSourceWidth() : "-";
//                    this.bodyProperties[11].value = cachedBody.getSpriteSourceHeight() != 0 ? cachedBody.getSpriteSourceHeight() : "-";
//                    this.bodyProperties[12].value = cachedBody.getSpriteOffsetX() != 0 ? cachedBody.getSpriteOffsetX() : "-";
//                    this.bodyProperties[13].value = cachedBody.getSpriteOffsetY() != 0 ? cachedBody.getSpriteOffsetY() : "-";
//                } else {
...
//
//                boolean allAreBullet = false;
//                boolean allHaveFixedRotation = false;
//                int  allHaveSameBodyType = 0;
//                for (int i = 0; i < sceneManager.selectedBodies.size(); i++) {
//                    if (allAreBullet != sceneManager.selectedBodies.get(i).isBullet && i != 0) {
//                        allAreBullet = false;
//                        break;
//                    } else {
//                        allAreBullet = sceneManager.selectedBodies.get(i).isBullet;
//                    }
//                }
//                for (int i = 0; i < sceneManager.selectedBodies.size(); i++) {
//                    if (allHaveFixedRotation != sceneManager.selectedBodies.get(i).isFixedRotation && i != 0) {
//                        allHaveFixedRotation = false;
//                        break;
//                    } else {
//                        allHaveFixedRotation = sceneManager.selectedBodies.get(i).isFixedRotation;
//                    }
//                }
//                for (int i = 0; i < sceneManager.selectedBodies.size(); i++) {
//                    if (allHaveSameBodyType != sceneManager.selectedBodies.get(i).bodyType && i != 0) {
//                        allHaveSameBodyType = 3;
//                        break;
//                    } else {
//                        allHaveSameBodyType = sceneManager.selectedBodies.get(i).bodyType;
//                    }
//                }
...
//        }
    ...
    public void updateJointPropertyView () {
//        if (sceneManager.state == SceneManager.STATE_DEFAULT_MODE &&
//                sceneManager.selectedJoints.size() > 0) {
//            $("#joint_properties").show();
//            String [] jointNames = {"Distance", "Weld", "Revolute", "Wheel", "Pulley", "Gear", "Prismatic", "Rope"};
//            if (sceneManager.selectedJoints.size() == 1) {
//                Joint cachedJoint = sceneManager.selectedJoints.get(0);
//                $(this.jointPropertyRows[2]).find("p")[1].innerHTML = jointNames[cachedJoint.jointType];
//                $(this.jointPropertyRows[this.jointPropertyRows.size() - 1]).show();
//
...
//                        $(this.jointPropertyRows[10]).find("p")[1].innerHTML = cachedJoint.lowerAngle;
//                        $(this.jointPropertyRows[11]).find("p")[1].innerHTML = cachedJoint.upperAngle;
//                    } else {
...
//                        $(this.jointPropertyRows[10]).find("p")[1].innerHTML = cachedJoint.lowerTranslation;
//                        $(this.jointPropertyRows[11]).find("p")[1].innerHTML = cachedJoint.upperTranslation;
//                    }
...
//        }
    ...
    public void updateInputXY(InputHandler inputHandler) {
//        if (inputHandler.transformTool == 5) {
//            if (this.sceneManager.state != SceneManager.STATE_SHAPE_EDIT_MODE) {
...
//        } else if (inputHandler.transformTool == 7) {
//            if (this.sceneManager.state == SceneManager.STATE_SHAPE_EDIT_MODE) {
//                if (inputHandler.selection.size() == 1 && inputHandler.selection[0].x) {
//                    this.xyInput[0].value = inputHandler.selection.get(0).x.toFixed(2);
...
//            }
//        } else if (inputHandler.transformTool == 6) {
//            if (this.sceneManager.state != SceneManager.STATE_SHAPE_EDIT_MODE && inputHandler.selection.get(0).rotation) {
...
//    public void  mixin(target, source, methods) {
